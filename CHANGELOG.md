...
- **Proximity duration lost on restart** — when HA restarted while a pair was in proximity, the elapsed time from `proximity_since` to the shutdown was silently discarded. Duration is now credited on restore, and `proximity_since` is advanced to `now` so the next EXIT does not double-count the same interval.
...
- **`TOTAL_INCREASING` state class on daily-reset sensors** — reverted: `TOTAL_INCREASING` is correct for HA long-term statistics on accumulator sensors that reset daily; `MEASUREMENT` (used in the earlier attempt) causes HA to compute mean/min/max instead of sums, making weekly totals impossible.
